...
public class RoleDAO implements DentalRepository<Role> {
    ...
    public ObservableList<Role> getAll() {
        ...
        try {
            String sql = "SELECT r.id, r.tile, r.slug, r.description, r.is_deleted, r.create_at, r.update_at " +
    ...
    public Role save(Role entity) {
        try {
            String sql = "INSERT INTO role (title, slug, description, create_at) " +
                    "VALUES (?, ?, ?, ?)";
    ...
    public Role update(Role entity) {
        try {
            String sql = "UPDATE role r " +
                    "SET r.title = ?, r.slug = ?, r.description = ?, r.update_at = ? " +
                    "WHERE r.id = ?";
            PreparedStatement pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, entity.getTitle());
            pstmt.setString(2, entity.getSlug());
